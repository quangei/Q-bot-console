...
function createbot() {
  ...
  rl.on('line', (input) => {
    let arg = input.split(' ')
    if (input.startsWith('.')) {
      let argrument = input.slice(1).split(' ');
      let cmd = argrument.shift().toLowerCase()
        switch(cmd) {
          case 'help':
            const cguis = [
              {
          ...
          case 'spam':
            let args = input.split(/ (?=(?:(?:[^"]*"){2})*[^"]*$)/);
            if (args.length < 3) {
              clientlog(`Usage: .${cmd} "<text>" <delay> <time> <antispamtype:length:customechars(optional)`)
              clientlog(`Anti spam type:all | number | letter | capitalize | lowercase | custom`)
              return
            }
            let text = input.match(/"([^"]+)"/)[1];
          ...
          case 'lookat':
            ...
            if (arg.length === 2) {
              ...
              clientlog(`Usage: .${cmd} <x,y,z> | <player>`);
              return;
            }
          ...
          case 'forcemove':
            if (arg[0] === '.forcemove') {
              ...
              if (direction && ['forward', 'back', 'left', 'right'].includes(direction) && !isNaN(duration) && duration > 0) {
                ...
                clientlog(`Usage: .${cmd} <Direction:forward|back|left|right> <duration>`);
                }
              }
            break;
          ...
          case 'sleep':
            ...
            async function goToSleep () {
              ...
              if (bed) {
                try {
                  await bot.pathfinder.setGoal(new GoalNear(bed, 2))
                  await bot.sleep(bed)
                  clientlog("I'm sleeping")
                } catch (err) {
                  clientlog(`I can't sleep: ${err.message}`)
                }
              } else {
                clientlog('No nearby bed')
              }
            }
            break;
          ...
          case 'wakeup':
            ...
            async function wakeUp () {
              try {
                ...
                clientlog(`I can't wake up: ${err.message}`)
              }
            }
            break;
          ...
          case 'login':
            options.username = arg[1]
            clientlog(`Change name to ${arg[1]}`)
            break;
  ...
  bot.on('sleep', () => {
    clientlog(`${options.username} sleep`)
  })
  bot.on('wake', () => {
    clientlog(`${options.username} wake up`)
  })
}
